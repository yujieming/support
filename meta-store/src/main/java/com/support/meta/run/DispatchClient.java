 ...
 */
public final class DispatchClient implements Closeable {
    //build the client
    ...
    private void run(int increment, boolean blocking) throws Exception {
        ...
        if (blocking) {
            ...
            for (int i = 0; i < increment; i++) {
                final Future<MetaReplyProto> f = executor.submit(() -> ratisClient.writeCover(write()));
...
//                final Future<RaftClientReply> f = client.async().send(Message.valueOf(write()));
                final Future<MetaReplyProto> f = ratisClient.writeAsyncCover(write());
                futures.add(f);
            }
        }
        ...
        for (Future<MetaReplyProto> f : futures) {
            final MetaReplyProto reply = f.get();
            boolean success = reply.getBase().getSuccess();
            System.out.println("writer is " + success);
        }

        MetaReplyProto replyProto = ratisClient.readCover(list("1"));
//        final RaftClientReply reply = client.io().sendReadOnly(Message.valueOf(read()));
        ListReplyProto list = replyProto.getList();
        int metasCount = list.getMetasCount();
        System.out.println("Current counter value: " + metasCount);
        for (Meta meta : list.getMetasList()) {
            System.err.println(" meta message : " + meta.getKey().toStringUtf8() + " value : " + meta.getData().toStringUtf8());
        }
    ...
    public static void main(String[] args) {
        try (DispatchClient client = new DispatchClient()) {
...
//            RaftClientReply raftClientReply = client.client.admin().setConfiguration(PEERS);
    ...
    private ByteString write() {
        ...
        HashMap<String, String> hashMap = new HashMap<String, String>() {{
            put(String.valueOf(random.nextInt(19)), UUID.randomUUID().toString());
        }};
